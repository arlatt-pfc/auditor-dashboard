  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h3 className="text-xl font-semibold text-slate-900">Expedientes Aduanales Auditados</h3>
          <p className="mt-1 text-sm text-slate-500">Expedientes integrales leidos desde customs_operations.</p>
        </div>
        <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-600">
          {operations.length} expedientes
        </span>
      </div>
            ...
            {operations.map((operation) => (
              <tr className="border-b border-slate-100 align-top last:border-b-0" key={operation.operationId}>
                <td className="py-4 pr-4">
                  ...
                  <div className="mt-1 text-xs text-slate-500">{operation.customsReference || "Sin referencia"}</div>
                ...
                <td className="py-4 pr-4 font-medium text-slate-900">{formatCurrency(operation.metrics.igiPaid)}</td>
